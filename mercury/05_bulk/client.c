...
#include <stdlib.h>
#include <mercury.h>
// #include "/home/xb/project/stor/daos/origin/docker/daos/build/external/debug/mercury/src/mercury.h"
#include "types.h"
...
int main(int argc, char** argv)
{
    ...
    ret = HG_Addr_lookup1(state.hg_context, lookup_callback, &save_op, server_address, HG_OP_ID_IGNORE);
...
hg_return_t lookup_callback(const struct hg_cb_info *callback_info)
{
    ...
    fseek(file, 0L, SEEK_END); // C 库函数 int fseek(FILE *stream, long int offset, int whence) 设置流 stream 的文件位置为给定的偏移 offset，参数 offset 意味着从给定的 whence 位置查找的字节数
    save_op->size = ftell(file); // C 库函数 long int ftell(FILE *stream) 返回给定流 stream 的当前文件位置
